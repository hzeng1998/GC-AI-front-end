...
class Register extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.name && this.state.password && this.state.checkPassword && this.state.email && this.state.file && this.state.verifyCode) {

      let {password, email, checkPassword, verifyCode} = this.state;
  ...
  handleVerifyCode = () => {
    if (this.state.email) {
      ...
      this.props.getVerifyCode(data);
    } else {
      console.log("Incomplete");
      this.props.setErrorMessage("Please Complete the Email Address");
    }
  };
          ...
          <Paper className={classes.paper}>
            ...
            <form className={classes.form} onSubmit={this.handleSubmit}>
              ...
              <Grid container justify={"flex-end"} alignItems={"center"}>
                <Grid item xs>
                  <FormControl margin="normal" required fullWidth>
                    <InputLabel htmlFor="Verify">Verify Code</InputLabel>
                    <Input id="verifyCode" name="verifyCode" value={this.state.verifyCode}
                ...
                <Grid item xs={6} sm={4}>
                  <Button
                    ...
                    variant={"contained"}
                    fullWidth
                    disabled={this.props.isFetchingVerifyCode}
            ...
            <Link to={"/login"}><Typography variant={"subtitle1"}>{"Already have an account? Sign in."}</Typography></Link>
          </Paper>
          <ErrorMessage/>
        </Grid>
      </Grid>
